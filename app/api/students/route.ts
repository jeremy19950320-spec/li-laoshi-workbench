...
async function init() {
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS teacher_students (
    ...
    guardian1_phone TEXT, guardian2_name TEXT, guardian2_relation TEXT, guardian2_phone TEXT,
...
export async function GET() {
  try {
    ...
    const { results } = await env.DB.prepare("SELECT id,name,grade,class_name as className,sex,student_no as studentNo,address,political,remark,status,guardian1_name as guardian1Name,guardian1_relation as guardian1Relation,guardian1_phone as guardian1Phone,guardian2_name as guardian2Name,guardian2_relation as guardian2Relation,guardian2_phone as guardian2Phone FROM teacher_students ORDER BY class_name, student_no").all();
...
export async function POST(request: Request) {
  try {
    ...
    const now = new Date().toISOString();
    const result = await env.DB.prepare("INSERT INTO teacher_students (name,grade,class_name,sex,student_no,address,political,remark,status,guardian1_name,guardian1_relation,guardian1_phone,guardian2_name,guardian2_relation,guardian2_phone,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(item.name,"九年级",prefix,item.sex,studentNo,item.address || "",item.political || "群众",item.remark || "",item.status || "正常",item.guardian1Name || "",item.guardian1Relation || "",item.guardian1Phone || "",item.guardian2Name || "",item.guardian2Relation || "",item.guardian2Phone || "",now).run();
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await env.DB.prepare("UPDATE teacher_students SET name=?, sex=?, address=?, political=?, remark=?, status=?, guardian1_name=?, guardian1_relation=?, guardian1_phone=?, guardian2_name=?, guardian2_relation=?, guardian2_phone=? WHERE id=?").bind(item.name,item.sex,item.address || "",item.political || "群众",item.remark || "",item.status || "正常",item.guardian1Name || "",item.guardian1Relation || "",item.guardian1Phone || "",item.guardian2Name || "",item.guardian2Relation || "",item.guardian2Phone || "",item.id).run();
